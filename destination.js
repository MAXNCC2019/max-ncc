...
function setLang(l){
  ...
  try{
    localStorage.setItem("maxncc-dest-lang",lang);
    localStorage.setItem("maxncc-lang",lang);
  }catch(e){}
}
function initialLang(){
  try{
    const q=new URLSearchParams(location.search).get("lang");
    if(q&&window.DEST_T&&window.DEST_T[q])return q;
    const saved=localStorage.getItem("maxncc-lang")||localStorage.getItem("maxncc-dest-lang");
    if(saved&&window.DEST_T&&window.DEST_T[saved])return saved;
  }catch(e){}
  return "it";
}
